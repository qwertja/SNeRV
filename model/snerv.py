...
class SNeRV(nn.Module):
    def __init__(self, args):
        ...
        self.decoder_len = len(args.dec_strds) + 1
    ...
    def forward(self, input, input_embed=None):
        ...
        if input_embed != None:
            ...
            embed_curr = self.encoder[0](embed)
            img_embed = [embed_curr, yl_norm]
        ...
        for layer in self.decoder[1:self.decoder_len]:
            output = layer(output)
            embed_list.append(output)
        ...
        yl_out = img_yl * (img_embed[1][1]-img_embed[1][0]) + img_embed[1][0]
...
# class SNeRV_T(nn.Module): # SNeRV_T with 2Dconv
#     def __init__(self, args):
#         super().__init__()
#         self.embed = args.embed
#         ks_enc, ks_dec1, ks_dec2 = [int(x) for x in args.ks.split('_')]
