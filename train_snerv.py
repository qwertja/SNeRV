...
def main():
    ...
    parser.add_argument('--model', type=str, default='snerv',choices=['snerv'], help='')
...
def evaluate(model, full_dataloader, local_rank, args, 
    ...
    for model_ind, cur_model in enumerate(model_list):
        ...
        for i, sample in enumerate(full_dataloader):
            ...
            img_out, embed_list, dec_time, _, _ = cur_model(img_data, 
                                            [dequant_vid_embed[i], dequant_vid_embed3[i]] if model_ind else None)
            if model_ind == 0:
                img_embed_list.append(embed_list[0][0])
                yl_norm_list.append(embed_list[0][1])
    ...
    if local_rank in [0, None] and quant_ckt != None:
        quant_vid = {'embed': quant_embed, 'norm': quant_embed3, 'model': quant_ckt}
